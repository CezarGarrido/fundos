use crate::services::analytics::compute_asset_analytics;
use polars::prelude::*;
...
mod tests {
    ...
    async fn test_real_cvm_backtest() {
        ...
        let quotes_df = DataFrame::new(vec![q_date, q_close]).unwrap();
        let yahoo_provider = crate::providers::yahoo::YahooProvider::new();
        ...
        for cnpj in fund_cnpjs {
            ...
            let top_series = crate::services::analytics::compute_top_series(&df, None);
            for series in top_series.into_iter().take(3) {
                ...
                if let Some(analytics) = crate::services::analytics::compute_asset_analytics(
                    &df,
                    &asset_name,
                    Some(quotes_to_use),
                    None,
                    ...
                    if let Some(inference) = analytics.portfolio_inference {
                        ...
                        if states.len() >= 2 {
                            ...
                            let matches = match inference.bias_direction {
                                crate::services::analytics::TradeBias::Acumulando => overall_delta > 0.0,
                                crate::services::analytics::TradeBias::Distribuindo => overall_delta < 0.0,
                                crate::services::analytics::TradeBias::Consistente => {
