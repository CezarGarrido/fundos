...
impl Portfolio {
    ...
    pub async fn async_read_assets(
        ...
        crate::providers::cvm::fund::set_status("Baixando composição de carteiras da CVM...");
        let result = self
            .options
            ...
            Ok(paths) => {
                ...
                for path in paths {
                    ...
                    for path in glob(&pattern).unwrap().filter_map(Result::ok) {
                        ...
                        crate::providers::cvm::fund::set_status(&format!(
    ...
    pub async fn async_market_assets(
        ...
        crate::providers::cvm::fund::set_status("Baixando carteiras do mercado para análise...");
        let (lf, _pl) = self
            .async_read_assets(start_date, end_date)
            .await
            .map_err(|e| PolarsError::NoData(e.to_string().into()))?;

        crate::providers::cvm::fund::set_status("Calculando ranking de ativos do mercado...");
